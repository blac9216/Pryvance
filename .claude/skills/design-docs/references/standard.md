...
An agent that auto-loads a skill or tool wanting a `docs/design/` or
`docs/superpowers/specs/` directory (e.g. the `superpowers` plugin's spec path) declines
that path; `doc-manifest.md` records the ruling so the decline has a citation.
