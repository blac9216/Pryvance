...
and the Tier 2 tasks listed. If `doc-manifest.md` is missing entirely it stops: audit a
repository that has not adopted, and every finding is "not adopted".

`--root` is a required argument — `audit.sh` never discovers the repository root itself,
and exits 2 naming the flag when it is absent.

Once `doc-manifest.md` exists, every line and section `audit.sh` reads from it is an
**agreed marker, not a default**: a missing one (a heading, a `Directory:`/`Index:`/
`domain model:` line) is a Tier 1 finding — `docs/doc-manifest.md MANIFEST_LINE_MISSING
<what is missing>` — that counts toward the exit-1 total exactly like any other finding,
never a silently-applied default and never a NOTE. The checks that line configures are
then skipped, and that skip is recorded in the `## Skipped checks` section below, not
folded away as a note either.
