...
records are never committed; a committed-specs architectural path (e.g. the
`superpowers` plugin's `brainstorming` flow) is not used here. Adopted under ADR-NNNN.
