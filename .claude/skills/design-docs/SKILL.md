...
- **Specs are never committed.** Designs are decided on GitHub (interrogate → plan-work);
  what survives the build is written into the design set. Any skill or tool that wants a
  `docs/design/` or `docs/superpowers/` directory (e.g. the `superpowers` plugin's
  brainstorming spec path) is overridden by the manifest: there is no such directory here.

## Scripts

All bash, shellcheck-clean, tested under `tests/` — run individually, see
[github-workflow/tests/README.md](../github-workflow/tests/README.md). `--root` is a
required argument on all three (never auto-discovered — see
[github-workflow/references/github-tools.md](../github-workflow/references/github-tools.md)
§ Extraction vs. interpretation and #736's no-discovery boundary: a script reads only
what it is given and only the framework's own agreed markers — `docs/doc-manifest.md`,
ADR `Status:` header fields, `# why:` pointers — never guessing a repository root or a
missing manifest line). Each exits 0 clean / 1 findings / 2 usage.

| Script | Class | Does | Used by |
|---|---|---|---|
| `scripts/check-pointers.sh --root R [--format text\|json]` | machine — reads agreed markers: `# why:` pointers / `### <slug>` + `Refs:` headings | every `# why:` pointer resolves; slugs unique; entry length and `Refs:` | audit; repo CI (adopt wires it) |
| `scripts/adr-index.sh --root R [--check\|--write]` | machine — reads agreed markers: `Status:` header fields / adr-index markers | generate the ADR status table; `--check` fails on drift, `--write` updates README | author (after any ADR change); audit; CI |
| `scripts/audit.sh --out <scratch-path> --root R` | machine — reads agreed markers: `docs/doc-manifest.md` section headings/lines, plus the two scripts above | runs Tier 1 checks, writes the gap report scaffold with Tier 2 agent tasks listed; a missing `docs/doc-manifest.md` line is a finding, never a default | audit |

`docs/doc-manifest.md` is the framework's own agreed marker: it is the repository's
adopted shape (which doc kinds exist, rationale areas, ADR range, Diátaxis directories),
so these scripts read it rather than guessing any of that from the tree.
...
A committed-specs architectural path (e.g. the `superpowers` plugin's `brainstorming` →
writing-plans flow) is **not** used in repositories that adopt this framework;
`doc-manifest.md` says so, and its useful ideas — the spike/bounded/architectural ratchet,
"design it twice", the three-part ADR trigger test — live in
