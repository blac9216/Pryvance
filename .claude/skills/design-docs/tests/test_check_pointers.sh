...
# conventions (report()/fail-counter, mutation-probe rule). check-pointers.sh takes
...
# tripwire.
#
# UNMOCKED-CONTEXT: this suite calls no `gh` — check-pointers.sh operates
# purely on local fixture files, so there is nothing to mock and no
# tripwire to wire; exempt per github-workflow/tests/README.md's
# UNMOCKED-CONTEXT convention.
#
# Mutation probe: case_missing_root below is the named probe for the
# --root-required refusal (#790) — it fails (wrong exit code) under the
# pre-change script, which defaulted --root via `git rev-parse
...
# the flag. This is the mutation probe for #790's --root-required change:
# it fails against the pre-change script, which defaulted --root instead.
# ---------------------------------------------------------------------------
case_missing_root() {
  set +e
  CP_STDOUT=$("$CHECK_POINTERS_SH" 2>"$WORK/stderr.log")
  CP_RC=$?
  set -e
  CP_STDERR="$(cat "$WORK/stderr.log")"
  [ "$CP_RC" -eq 2 ] || report "missing_root: expected exit 2, got $CP_RC"
  grep -qF -- "--root" <<<"$CP_STDERR" || report "missing_root: expected the error to name --root, got: $CP_STDERR"
}
