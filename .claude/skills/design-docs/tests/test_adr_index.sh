...
# conventions (report()/fail-counter, mutation-probe rule). adr-index.sh takes
...
# tripwire.
#
# UNMOCKED-CONTEXT: this suite calls no `gh` — adr-index.sh operates purely
# on local fixture files, so there is nothing to mock and no tripwire to
# wire; exempt per github-workflow/tests/README.md's UNMOCKED-CONTEXT
# convention.
#
# Mutation probe: the "missing --root" case below is the named probe for
# the --root-required refusal (#790) — it fails (wrong exit code) under
# the pre-change script, which defaulted --root via `git rev-parse
# --show-toplevel` instead of requiring it.
#
...
'
if actual="$("$ADR_INDEX_SH" --root "$d" --adr-dir "$d")" && [[ "$actual" == "$expected_table_clean" ]]; then
...
<!-- adr-index:end -->
EOF
if "$ADR_INDEX_SH" --root "$d" --adr-dir "$d" --check >"$WORK/case2.out" 2>"$WORK/case2.err"; then
...
EOF
set +e
"$ADR_INDEX_SH" --root "$d" --adr-dir "$d" --check >"$WORK/case3.out" 2>"$WORK/case3.err"
rc=$?
set -e
...
printf '# ADRs\n\nno markers here\n' >"$d/README.md"
set +e
"$ADR_INDEX_SH" --root "$d" --adr-dir "$d" --check >"$WORK/case3b.out" 2>"$WORK/case3b.err"
rc=$?
set -e
...
EOF
if "$ADR_INDEX_SH" --root "$d" --adr-dir "$d" --write >/dev/null \
   && "$ADR_INDEX_SH" --root "$d" --adr-dir "$d" --check >/dev/null 2>"$WORK/case4.err" \
...
printf '# ADRs\n' >"$d/README.md"
if "$ADR_INDEX_SH" --root "$d" --adr-dir "$d" --write >/dev/null \
   && "$ADR_INDEX_SH" --root "$d" --adr-dir "$d" --check >/dev/null 2>"$WORK/case4b.err"; then
...
'
set +e
"$ADR_INDEX_SH" --root "$d" --adr-dir "$d" >"$WORK/case5.out" 2>"$WORK/case5.err"
rc=$?
set -e
...
'
set +e
"$ADR_INDEX_SH" --root "$d" --adr-dir "$d" >"$WORK/case6.out" 2>"$WORK/case6.err"
rc=$?
set -e
...
'
set +e
"$ADR_INDEX_SH" --root "$d" --adr-dir "$d" >"$WORK/case7.out" 2>"$WORK/case7.err"
rc=$?
set -e
...
'
set +e
"$ADR_INDEX_SH" --root "$d" --adr-dir "$d" >"$WORK/case8.out" 2>"$WORK/case8.err"
rc=$?
set -e
if [[ "$rc" -ne 0 ]] \
...
'
set +e
"$ADR_INDEX_SH" --root "$d" --adr-dir "$d" >"$WORK/case9.out" 2>"$WORK/case9.err"
rc=$?
set -e
...
New decision.
'
if actual="$("$ADR_INDEX_SH" --root "$d" --adr-dir "$d")" \
...
'
set +e
"$ADR_INDEX_SH" --root "$d" --adr-dir "$d" >"$WORK/case11.out" 2>"$WORK/case11.err"
rc=$?
set -e
...
set +e
"$ADR_INDEX_SH" --root "$d" --adr-dir "$d" --write >"$WORK/case12.out" 2>"$WORK/case12.err"
rc=$?
set -e
...
set +e
"$ADR_INDEX_SH" --root "$d" --adr-dir "$d" >"$WORK/empty.out" 2>"$WORK/empty.err"
rc=$?
set -e
...
# ---------------------------------------------------------------------------
# Case: --root is required; omitting it is a usage error (exit 2), naming
# the flag.
d="$(fresh_dir rootreq)"
set +e
"$ADR_INDEX_SH" --adr-dir "$d" >"$WORK/rootreq.out" 2>"$WORK/rootreq.err"
rc=$?
set -e
if [[ "$rc" -eq 2 ]] && grep -qF -- "--root" "$WORK/rootreq.err"; then
  pass "adr-index.sh without --root exits 2 and names the flag"
else
  fail "adr-index.sh without --root: expected exit 2 naming --root, got rc=$rc: $(cat "$WORK/rootreq.err")"
fi
