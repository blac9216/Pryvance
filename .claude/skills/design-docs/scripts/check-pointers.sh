...
# (including a missing --root: this script never discovers the repository
# root on its own — no auto-detection fallback — the caller always names
# it). A one-line summary (`check-pointers: N findings`) always goes to
# stderr.
#
# Class: machine — reads agreed markers: `# why:` (also `// why:` /
# `<!-- why: -->`) pointers in code and the `### <slug>` headings plus
# `Refs:` lines they must resolve to in docs/rationale/*.md.
set -euo pipefail
...
if [ -z "$ROOT" ]; then
  echo "check-pointers: --root is required" >&2
  usage
  exit 2
fi
if [ ! -d "$ROOT" ]; then
