...
#   1  Tier 1 findings reported (including a missing docs/doc-manifest.md
#      line — MANIFEST_LINE_MISSING — for every line the script reads)
#   2  usage error (--out or --root missing/invalid), or docs/doc-manifest.md
#      is missing entirely (the repository has not adopted the standard) —
#      a minimal report is still written
#
# --root is required: this script never discovers the repository root on
# its own (no auto-detection fallback) — the caller always names it.
#
...
# stdout.
#
# Class: machine — reads agreed markers: docs/doc-manifest.md's own section
# headings and `Directory:`/`Index:`/`domain model:` lines, plus ADR
# `Status:` header fields via adr-index.sh and `# why:` pointers via
# check-pointers.sh. A missing marker is a finding, never a default.
set -euo pipefail
...
if [ -z "$ROOT" ]; then
  echo "audit.sh: --root is required" >&2
  usage
  exit 2
fi
if [ ! -d "$ROOT" ]; then
...
# whitespace; a missing line or section is never defaulted or silently
# skipped — it is a MANIFEST_LINE_MISSING finding (exit 1), and every
# dependent check it configures is recorded in SKIPPED_CHECKS, which the
# report renders as its own finding-bearing section (never a NOTE).
# ---------------------------------------------------------------------------
DOC_STANDARD_REL="${DOC_STANDARD#"$ROOT"/}"
declare -a NOTES=()
...
if grep -q '^## Design set' "$DOC_STANDARD"; then
  ...
  add_finding "$DOC_STANDARD_REL" "" "MANIFEST_LINE_MISSING" "'## Design set' section missing"
...
if [ -n "$diataxis_line" ]; then
  ...
  add_finding "$DOC_STANDARD_REL" "" "MANIFEST_LINE_MISSING" "Diátaxis directories line (tutorials:/how-to:/reference:/explanation:) missing"
...
if [ -n "$index_line" ]; then
  ...
  add_finding "$DOC_STANDARD_REL" "" "MANIFEST_LINE_MISSING" "'Index:' line missing"
fi
...
if [ -n "$adr_line" ]; then
  ...
  add_finding "$DOC_STANDARD_REL" "" "MANIFEST_LINE_MISSING" "'Directory:' line (ADRs) missing"
  SKIPPED_CHECKS+=("ADR index / MADR section checks — needs the ADRs section's 'Directory:' line in doc-manifest.md")
fi
...
if grep -q '^## Rationale areas' "$DOC_STANDARD"; then
  ...
  add_finding "$DOC_STANDARD_REL" "" "MANIFEST_LINE_MISSING" "'## Rationale areas' section missing"
fi
...
if [ -n "$glossary_line" ]; then
  ...
  add_finding "$DOC_STANDARD_REL" "" "MANIFEST_LINE_MISSING" "Glossary line (CONTEXT.md at repo root) missing"
  SKIPPED_CHECKS+=("Glossary checks — needs the Glossary section's 'CONTEXT.md at repo root' line in doc-manifest.md")
fi
...
if [ -n "$domain_line" ]; then
  ...
  add_finding "$DOC_STANDARD_REL" "" "MANIFEST_LINE_MISSING" "'domain model:' line missing"
...
if { [ -x "$ADR_INDEX_SH" ] || [ -f "$ADR_INDEX_SH" ]; } && [ -n "$ADR_DIR" ]; then
...
# ---------------------------------------------------------------------------
if [ -n "$ADR_DIR" ] && [ -d "$ROOT/$ADR_DIR" ]; then
  shopt -s nullglob
  for adr in "$ROOT/$ADR_DIR"/[[:digit:]]*.md; do
    ...
    # Capture the header block before grepping: `head | grep -q` under pipefail can
