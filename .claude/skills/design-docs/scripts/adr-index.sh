...
#
# Exit status: 0 = clean (or --check/--write succeeded), 1 = findings
# reported, 2 = usage error — including a missing --root, which this
# script never discovers on its own (no auto-detection fallback; the
# caller always names it).
#
# --adr-dir is relative to --root unless given as an absolute path, and
# itself defaults to docs/adr. If the resolved ADR directory does not
# exist, that is not a usage error: adr-index.sh treats it as zero ADRs —
# the generated table is "0 ADRs" plus an empty (headers-only) table, and
# each mode proceeds normally against it (print prints it, --check
# compares it to README.md as usual, --write writes it as usual).
# Only a malformed command line (bad flag, missing/empty argument,
# including a missing --root) is a usage error (exit 2).
#
# Class: machine — reads agreed markers: each ADR's `Status:` header field
# (plus `Supersedes:`/`Superseded-by:`/`Amends:`/`Amended-by:`) and the
# `<!-- adr-index:start/end -->` markers in <adr-dir>/README.md.
#
...
if [[ -z "$root_arg" ]]; then
  echo "adr-index.sh: --root is required" >&2
  usage
  exit 2
fi
root="$root_arg"
