...
This is the single-sourced text of the rules every dispatched agent follows. Each
`workflow-*` definition mirrors the blocks below verbatim inside marked sections; when a
definition does not resolve at dispatch time, the orchestrator splices this file's
content into the dispatch prompt directly so the rules still reach the agent. Edit the
rule here, then propagate — never hand-edit a mirrored copy out of sync with this file.

Every block below opens with an HTML comment naming its slug and closes with a matching
end comment, so a drift check can diff each mirrored copy against its source block.

Each definition also declares the set it mirrors, machine-readably, in a single
`<!-- mirrors: <slug>, <slug>, … -->` line directly below its frontmatter.
`tests/test_agent_rules_drift.sh` holds the declaration and the file to each other in
both directions: every declared slug must be present and byte-identical, and every block
present must be declared. Deleting a whole block from a definition therefore fails the
check instead of passing silently — add or remove a slug from the declaration line in the
same commit that adds or removes its block, and say in the role intro why any omitted
block does not apply to that role.
