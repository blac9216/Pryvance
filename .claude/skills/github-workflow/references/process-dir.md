...
| File | Holds | Read at |
|---|---|---|
| `work-tracking.md` | the four-layer shape as adopted here, any local rule deviations, Project number/ids; a `Session-log archive:` line (`owner/name`, or `none — <why>`) that `save-log.sh` reads | Orient; `status`, `good morning`, `save`, Close checklist |
| `worktrees.md` | the worktree root and the per-issue / per-review naming under it | Orient, every dispatch |
...
| `maintenance.md` | scratch dir, allowed file locations, test resource prefixes, host thresholds, shared sequence resources (e.g. migration numbering) | Maintenance, dispatch |
