...
formatting passes, and other maintenance work. Apply the `chore` label, a `size:*`
label, and at least one `area:*` label at creation.
...
non-code issue (no net LOC — most chores that are docs-only or config-only) sizes by
review effort instead: S = a single file or section, readable in one pass; M = several
files/sections needing cross-checking; L = a structural rewrite spanning most of a
document or config set — split before filing, same as code. `size:l` on a filed issue
is a triage tripwire, not a resting state: since L already means "must be split before
