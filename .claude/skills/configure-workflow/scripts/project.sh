    ...
    if [ "$curf" != "$filter" ] || [ "$curcols" != "$wantcols" ] || [ "$(jq -r .layout <<<"$cur")" != "$layout" ]; then drift; say "correct view $name"
      if [ $AUDIT = 0 ]; then
        if [ "$layout" = ROADMAP_LAYOUT ]; then
          gql 'mutation($v:ID!,$f:String!){updateProjectV2View(input:{viewId:$v,filter:$f}){projectV2View{id}}}' "$(jq -n --arg v "$(jq -r .id <<<"$cur")" --arg f "$filter" '{v:$v,f:$f}')" >/dev/null
        else
          gql 'mutation($v:ID!,$f:String!,$l:ProjectV2ViewLayout!,$ids:[ID!]){updateProjectV2View(input:{viewId:$v,filter:$f,layout:$l,configuration:{visibleFieldIds:$ids}}){projectV2View{id}}}' "$(jq -n --arg v "$(jq -r .id <<<"$cur")" --arg f "$filter" --arg l "$layout" --argjson ids "$ids" '{v:$v,f:$f,l:$l,ids:$ids}')" >/dev/null
        fi
      fi
    fi
  fi
