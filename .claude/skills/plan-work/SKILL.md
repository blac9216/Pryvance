...
1. **Orient.** Read `docs/process/*.md` and `*.local.md` if present, the board, the open
   ...
   `scripts/history.sh --repo <owner/name>` once — `--repo` is a required argument, never
   discovered, and one run completes its collection inside a single API-quota window,
   resuming where it left off if interrupted — it builds the calibration table the
   estimates use ([references/estimate.md](references/estimate.md)).
...
- **Scripts take every input as an argument.** `scripts/*` never discover their own
  configuration or interpret prose — see
  [github-workflow/references/github-tools.md](../github-workflow/references/github-tools.md)
  § Extraction vs. interpretation, and #736's no-discovery boundary outside it.
