...
3. **Dispatch lanes** as background agents (mid tier (Claude: Sonnet) by default; large
   tier (Claude: Opus) only for a lane that must reason about security or contracts).
   Each lane writes its **findings comment**
