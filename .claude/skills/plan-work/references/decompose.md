...
milestone, no new epic — and the decision record — posted with
`.claude/skills/github-workflow/scripts/post-comment.sh <epic> <body-file>` (by hand:
`gh api -X POST repos/<o>/<r>/issues/<epic>/comments -F body=@<file>`) — goes on that
epic's thread. A new milestone exists only for a story that spans more than one domain
epic and fits none of the open ones; a new epic only when a domain has no home. Creating
structure that already exists is the most common planning error and the hardest to undo.

**Milestone** — from github-workflow's `milestone-description` template, set via
`gh api -X PATCH repos/<o>/<r>/milestones/<n> -F description=@<file>` (no MCP/script
posting path exists for a milestone description — this is the by-hand cloud call
itself). Written before anything is assigned to it; `due_on` set by the projection
step. Only feature stories get one; hardening, CI and tooling stay milestone-less.
...
and a design pointer; the decision record from the interrogation is posted the same way
as above, on the design-record epic's thread.
