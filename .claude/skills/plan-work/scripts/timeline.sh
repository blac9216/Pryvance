...
# --repo is REQUIRED (exit 2 when absent, before any gh call) — this script never guesses
# the target repository (no `gh repo view` fallback).
# Reads each open milestone's open issues: hours per issue come from the issue's own
# `size:*` label (S/M/L) mapped through --defaults (an agent-passed table; the agent
# reads the calibration table history.sh wrote and passes the p50 per size) and
# `blocked_by` links (REST). An issue with no `size:*` label is reported on stderr by
# number and uses the M default, exactly as before — that is the only fallback, and it
# is a label-absence fact, not a parse: this script never reads issue-body prose.
...
# numbers, and numbers above the MAX_HOURS sanity ceiling (100000) are rejected (exit 2).
# Exit codes: 2 = argument error (unknown flag, a value-taking flag with no following value, a missing
# --repo, an empty --milestones/--milestone value, or a --defaults value that is empty or has a part
# that isn't S=<n>/M=<n>/L=<n> with n an ASCII decimal number greater than zero and no greater than
# MAX_HOURS (100000), so a non-ASCII digit such as "٢" is an argument error too); 3 = a
...
# was detected while computing a milestone's critical path (jq's own error exit surfaces here) — the
# only cause reachable from this script's own inputs; no --defaults value can produce it (it is either
# rejected with exit 2 or completed from the built-in table), but exit 5 is jq's generic error exit, so
# an unexpected jq failure would also surface as 5. No other exit code exists — every code above is
# reachable from some input, and no input falls through to a code not named here.
# MAX_HOURS (100000) bounds every numeric magnitude this script accepts -- --defaults S/M/L values,
# --parallelism, and a history-dir parallelism.txt value -- because jq 1.6 SIGABRTs (undocumented exit
# 134) when a sufficiently large number (roughly 15+ digits) reaches its per-milestone projection
# stage. Nothing near 100000 hours (~11 years) or that much parallelism is a plausible planning input,
# so the ceiling costs nothing real. A flag value above it is an argument error (exit 2 for --defaults,
# exit 4 for --parallelism); a parallelism.txt value above it falls back to the 1.5 default like any
# other unusable file value.
set -euo pipefail
...
# and a history-dir parallelism.txt value); see the header comment above.
MAX_HOURS=100000
MILESTONE_ARGS=()
...
[ -n "$REPO" ] || { echo "timeline.sh: --repo is required" >&2; exit 2; }
...
while IFS= read -r ms; do
  ...
  gh api --paginate "repos/$REPO/issues?milestone=$mn&state=all&per_page=100" --jq '.[]|select(.pull_request==null)|{number,state,created_at,closed_at,assignee:(.assignee.login//null),labels:[.labels[].name]}' | while IFS= read -r iss; do
    n=$(jq -r .number <<<"$iss")
    blocked=$(gh api "repos/$REPO/issues/$n/dependencies/blocked_by" --jq '[.[].number]' 2>/dev/null || echo '[]')
    # $size reads the issue's own `size:s`/`size:m`/`size:l` label (case-sensitive, the
    # canonical lowercase form) — the only per-issue input this script reads. No issue-body
    # prose is ever consulted. A repeated size label (should never occur) resolves to the
    # first one jq's array order returns.
    rec=$(jq -c --argjson ms "$ms" --argjson blocked "$blocked" --arg S "$defS" --arg M "$defM" --arg L "$defL" '
      ([.labels[]|select(test("^size:[sml]$"))]|.[0]|if . then (ltrimstr("size:")|ascii_upcase) else null end) as $size |
      {milestone:$ms.number, milestone_title:$ms.title, issue:.number, state:.state, assignee:.assignee, epic:([.labels[]|select(.=="epic")]|length>0),
       size:$size, hours:(if $size=="S" then ($S|tonumber) elif $size=="M" then ($M|tonumber) elif $size=="L" then ($L|tonumber) else ($M|tonumber) end),
       hours_source:(if $size!=null then "label-default" else "no-label-default-M" end),
       blocked_by:$blocked, created:.created_at, closed:.closed_at}' <<<"$iss")
    if [ "$(jq -r '.hours_source' <<<"$rec")" = "no-label-default-M" ]; then
      say "timeline: issue #$n has no size:* label; falling back to the M default"
    fi
